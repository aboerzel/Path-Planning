    ...
    const vector<vector<double>>& sensor_fusion)
{
    ...
    const auto min_stopping_distance = get_min_stopping_distance(current_speed, target_accel, lead_vehicle[1], 10.0) + MIN_DISTANCE;
    if (lead_vehicle[0] <= min_stopping_distance)
    {
...
bool HighwayDrivingBehavior::is_lane_safe_for_change(
    const double current_s, const int current_lane, const int lane, const double current_speed,
    const vector<vector<double>>& sensor_fusion)
{
    // check min stopping distance to the leading vehicle on current lane
    auto lead_vehicle = find_closest_vehicle(current_s, current_lane, sensor_fusion, true);
    auto min_stopping_distance = get_min_stopping_distance(current_speed, target_accel, lead_vehicle[1], 10.0);
    min_stopping_distance = min_stopping_distance > 0 ? min_stopping_distance + MIN_DISTANCE : MIN_DISTANCE;
    if (lead_vehicle[0] < min_stopping_distance)
    {
        printf("BLOCKING: Distance %4.2f m to vehicle ahead on lane %d < %4.2f m\n", lead_vehicle[0], current_lane,
               min_stopping_distance);
        return false;
    }

    // check min stopping distance to the leading vehicle on target lane
    auto vehicle_ahead = find_closest_vehicle(current_s, lane, sensor_fusion, true);
    auto vehicle_speed = sqrt(pow(vehicle_ahead[3], 2) + pow(vehicle_ahead[4], 2));
    min_stopping_distance = get_min_stopping_distance(current_speed, target_accel, vehicle_speed, 10.0);
    min_stopping_distance = min_stopping_distance > 0 ? min_stopping_distance + MIN_DISTANCE : MIN_DISTANCE;
    if (vehicle_ahead[0] < min_stopping_distance)
    {
        printf("BLOCKING: Distance %4.2f m to vehicle ahead on lane %d < %4.2f m\n", vehicle_ahead[0], lane,
               min_stopping_distance);
        return false;
    }

    // check min stopping distance to the vehicle behind on target lane
    auto vehicle_behind = find_closest_vehicle(current_s, lane, sensor_fusion, false);
    vehicle_speed = sqrt(pow(vehicle_behind[3], 2) + pow(vehicle_behind[4], 2));
    min_stopping_distance = get_min_stopping_distance(vehicle_speed, 10.0, current_speed, target_accel);
    min_stopping_distance = min_stopping_distance > 0 ? min_stopping_distance + 15 : 15;
    if (vehicle_behind[0] < min_stopping_distance)
    {
        printf("BLOCKING: Distance %4.2f m to vehicle behind on lane %d < %4.2f m\n", vehicle_behind[0], lane,
               min_stopping_distance);
        return false;
    }
    ...
    const double current_s, const int lane, const vector<vector<double>>& sensor_fusion)
{
    ...
    printf("%-32s: %4.2f m/s (%4.2f MPS)\n", ("lead vehicle speed    [lane " + to_string(lane) + "]").c_str(),
           lead_vehicle_speed,
...
double HighwayDrivingBehavior::get_min_stopping_distance(const double v1, const double a1, const double v2,
                                                         const double a2)
{
    return (v1 * v1 / a1) - (v2 * v2 / a2);
}
